        ...
        <div class="fl-left avatar-box">
          <div class="user-card">
            ...
            <div class="user-personality">
              <p v-if="!editFlag" class="realName">
...
import { setSelfInfo, changePassword, changeInfo } from '@/api/user.js'
...
const changeInfoReq = async(params) => {
  try {
...
const savePassword = async() => {
  modifyPwdForm.value.validate((valid) => {
    if (valid) {
      changePassword({
        oldPwd: pwdModify.value.password,
        newPwd: pwdModify.value.newPassword,
      }).then((res) => {
        if (res.code === 200) {
...
const enterImg = async(url) => {
  const res = await setSelfInfo({ image: url })
  if (res.code === 0) {
    userStore.ResetUserInfo({ image: url })
    ElMessage({
      type: 'success',
      message: '设置成功',
    })
...
const enterEdit = async() => {
  ...
  console.log(res)
  if (res.code === 200) {
    userStore.ResetUserInfo({ realName: realName.value })
    ElMessage({
      type: 'success',
      message: '更改成功',
    })
